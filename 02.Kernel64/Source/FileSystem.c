...
static int kFindFreeDirectoryEntry( void )
{
    ...
    // 루트 디렉터리를 읽음
    if( kReadCluster( currentDir, gs_vbTempBuffer ) == FALSE )
    {
        return -1;
    }
 ...
 */
static BOOL kSetDirectoryEntryData( int iIndex, DIRECTORYENTRY* pstEntry, int currentCluster )
{
    ...
    // 루트 디렉터리에 씀
    if( kWriteCluster( currentCluster, gs_vbTempBuffer ) == FALSE )
    {
        return FALSE;
    }    
    return TRUE;
        ...
        int* piDirectoryEntryIndex )
{
    ...
    *piDirectoryEntryIndex = kFindFreeDirectoryEntry();
    ...
    pstEntry->isFile=TRUE;
    
    // 디렉터리 엔트리를 등록
    if( kSetDirectoryEntryData( *piDirectoryEntryIndex, pstEntry, currentDir ) == FALSE )
    {
        // 실패할 경우 할당 받은 클러스터를 반환해야 함
        kSetClusterLinkData( dwCluster, FILESYSTEM_FREECLUSTER );
        return FALSE;
...
static BOOL kCreateDir( const char* pcFileName, DIRECTORYENTRY* pstEntry, 
        int* piDirectoryEntryIndex )
{
    DWORD dwCluster;
...
FILE* kOpenFile( const char* pcFileName, const char* pcMode )
{
    ...
    else if( pcMode[ 0 ] == 'w' )
    {
        ...
        if( kSetDirectoryEntryData( iDirectoryEntryOffset, &stEntry, currentDir ) == FALSE )
        {
            // 동기화
            kUnlock( &( gs_stFileSystemManager.stMutex ) );
            return NULL;
...
DWORD kNewDir(const char* pcFileName){
    DIRECTORYENTRY stEntry, pstEntry0={0},pstEntry1={0};
    int iDirectoryEntryOffset;
    int iFileNameLength;
    DWORD dwSecondCluster;
    FILE* pstFile;
    ...
    if( iDirectoryEntryOffset == -1  )
    {
        ...
        // 디렉터리 엔트리를 설정
        pstEntry0.vcFileName[0] =  '.';
        pstEntry0.dwStartClusterIndex = stEntry.dwStartClusterIndex;
        pstEntry0.dwFileSize = 0;
        pstEntry0.isFile=FALSE;
    
        // 디렉터리 엔트리를 등록
        if( kSetDirectoryEntryData( 0 , &pstEntry0, stEntry.dwStartClusterIndex ) == FALSE )
        {
            kPrintf("Failed");
            // 실패할 경우 할당 받은 클러스터를 반환해야 함
            kSetClusterLinkData( stEntry.dwStartClusterIndex, FILESYSTEM_FREECLUSTER );
            kUnlock( &( gs_stFileSystemManager.stMutex ) );
            return FALSE;
        }
        ...
        pstEntry1.vcFileName[0] = '.';
        pstEntry1.vcFileName[1] = '.';
        pstEntry1.dwStartClusterIndex = currentDir;
        pstEntry1.dwFileSize = 0;
        pstEntry1.isFile=FALSE;
        // 디렉터리 엔트리를 등록
        if( kSetDirectoryEntryData( 1 , &pstEntry1, stEntry.dwStartClusterIndex ) == FALSE )
        {
            kPrintf("Failed");
            // 실패할 경우 할당 받은 클러스터를 반환해야 함
            kSetClusterLinkData( stEntry.dwStartClusterIndex, FILESYSTEM_FREECLUSTER );
...
static BOOL kUpdateDirectoryEntry( FILEHANDLE* pstFileHandle )
{
    ...
    // 변경된 디렉터리 엔트리를 설정
    if( kSetDirectoryEntryData( pstFileHandle->iDirectoryEntryOffset, &stEntry, currentDir )
            == FALSE )
    {
        return FALSE;
    }
...
struct kDirectoryEntryStruct* kReadDirectory( DIR* pstDirectory )
{
    ...
    while( pstDirectoryHandle->iCurrentOffset < FILESYSTEM_MAXDIRECTORYENTRYCOUNT )
    {
        ...
        if( (pstEntry[ pstDirectoryHandle->iCurrentOffset ].dwStartClusterIndex
                != 0) ||(pstEntry[ pstDirectoryHandle->iCurrentOffset ].vcFileName[0]!=0) )
        {
            // 동기화
            kUnlock( &( gs_stFileSystemManager.stMutex ) );
