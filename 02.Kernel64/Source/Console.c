...
int kConsolePrintString( const char* pcBuffer )
{
    ...
    for( i = 0 ; i < iLength ; i++ )
    {
        ...
        if( iPrintOffset >= ( CONSOLE_HEIGHT - 2) * CONSOLE_WIDTH )
        {
            // 가장 윗줄을 제외한 나머지를 한줄 위로 복사
            kMemCpy( CONSOLE_VIDEOMEMORYADDRESS, 
                     CONSOLE_VIDEOMEMORYADDRESS + (CONSOLE_WIDTH) * sizeof( CHARACTER ),
                     ( CONSOLE_HEIGHT - 3 ) * (CONSOLE_WIDTH)* sizeof( CHARACTER ) );

            // 가장 마지막 라인은 공백으로 채움
            for( j = ( CONSOLE_HEIGHT -3 ) * ( CONSOLE_WIDTH) ; 
                 j < ( (CONSOLE_HEIGHT - 2 ) * CONSOLE_WIDTH ) ; j++ )
            {
                // 공백 출력
            ...
            iPrintOffset = ( CONSOLE_HEIGHT - 3 ) * CONSOLE_WIDTH;
        }
    }
    return iPrintOffset;
}
